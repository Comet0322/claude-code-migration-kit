...
def parse_edges(units: dict):
    ...
    for name, path in units.items():
        # Real-world legacy sources are not guaranteed to be UTF-8 (this batch's
        # files carry stray Windows-1251/Latin-1 bytes in comments); the `uses`
        # clause itself is always plain ASCII, so a lossless byte-preserving
        # decode is enough and avoids spurious UnicodeDecodeError aborts.
