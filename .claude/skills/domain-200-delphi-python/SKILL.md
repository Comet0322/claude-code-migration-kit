...
description: >
  ...
  （私有套件、Fingerprint、執行環境）共用 skill `legacy-200-delphi`（跟
  domain-200-delphi-java 是同一批來源）。目標端 DB/logging 用法共用 skill
  `python-corplib`；專案形狀（FastAPI 服務 / ETL 批次）依實例而定，見
  `python-corplib-fastapi` / `python-corplib-etl`（跟
  domain-300-vb-python 共用同一組目標 Python 形狀選項）。
---
...
來源見 skill `legacy-200-delphi`、目標見 skill `python-corplib`：

| Delphi（legacy-200-delphi） | Python（python-corplib） | 備註 |
|---|---|---|
